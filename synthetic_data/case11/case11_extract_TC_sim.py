...
# -------------------------------
data_path: Path = Path("case11_sim_out.e")
...
err_chain: list[sens.IErrSimulator] = [
    sens.ErrSysGen(sens.GenUniform(low=-20.0,high=20.0)),
    sens.ErrRandGen(sens.GenNormal(std=5.0), err_dep=EErrDep.DEPENDENT),
]
...
exp_sim_opts = sens.ExpSimOpts(workers=16,para=sens.EExpSimPara.ALL)
...
if pv_plot.off_screen:
    pv_plot.screenshot(output_path/"locs_sim.png")
else:
    pv_plot.show()
...
fig.savefig(output_path/"traces_sim.png",dpi=300,bbox_inches="tight")
...
np.savetxt(
    "pyvale-output/SamplingResultsOnlyPointSensors_sim.csv",
    exp_arr,
    delimiter=",",
    header=",".join(headers),
    comments=""
