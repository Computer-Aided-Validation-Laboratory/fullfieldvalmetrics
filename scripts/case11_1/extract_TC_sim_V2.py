...
params_json = (
   Path.cwd() / "synthetic_data/case11" / "pyvale-output/sims/sim-workdir-1/sweep-vars-1.json"
)
files_json = (
    Path.cwd() / "synthetic_data/case11" / "pyvale-output/sims/sim-workdir-1/output-key-1.json"
)

output_path = Path.cwd() / "synthetic_data/case11_1" / "pyvale-output"
...
for ii, w_cond in enumerate(wThermCond_vals):
    ...
    for jj, heat_flux in enumerate(surfHeatFlux_vals):
        ...
        if tc_values.shape != (sens_pos.shape[0],):
            raise ValueError(
                f"Unexpected TC shape for "
                f"wThermCond={w_cond}, "
                f"surfHeatFlux={heat_flux}: "
                f"{tc_values.shape}. "
                f"Expected ({sens_pos.shape[0]},)."
            )
...
headers = [
    "wThermCond",
    "surfHeatFlux",
] + [f"TC{n}" for n in range(1, sens_pos.shape[0] + 1)]
...
headers = ["x", "y", "z"]
np.savetxt(
    output_path / "PointSensorCoords_sim.csv",
    sens_pos,
    delimiter=",",
    header=",".join(headers),
    comments=""
