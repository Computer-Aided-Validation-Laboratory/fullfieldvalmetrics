...
def main() -> None:
    ...
    #f"temp_exp{EXP_TAG}_sim{SIM_TAG}_coord{COORD_TAG}"
    if not temp_path.is_dir():
        temp_path.mkdir(exist_ok=True,parents=True)

    save_path = Path.cwd() / f"images_dic_pulse25Xall_exp{EXP_TAG}_sim{SIM_TAG}"
    ...
    this_sim_path = (Path.cwd() 
                    / f"temp_exp{DIC_PULSES[0]}_sim{SIM_TAG}_coord{COORD_TAG}")
    ...
    exp_data = []
    for ee in range(3):
        this_exp_path = (Path.cwd() 
                        / f"temp_exp{DIC_PULSES[ee]}_sim{SIM_TAG}_coord{COORD_TAG}")
    ...
    for ax_ind,ax_str in enumerate(STRAIN_COMP_STRS):
        ...
        strain_color_max = np.nanmax((np.nanmax(sim_strain_grid_avg[:,:,ax_ind]),
                                      np.nanmax(exp_strain_grid_avg[:,:,ax_ind])))
        strain_color_min = np.nanmin((np.nanmin(sim_strain_grid_avg[:,:,ax_ind]),
        ...
        fig_size = (plot_opts.a4_print_width,
                    plot_opts.a4_print_width/(plot_opts.aspect_ratio*2.0))
        ...
        image = ax[4].imshow(mavm_dm_grid,
            ...
            cmap="plasma")
        d_str = r"$d^{-}$"
        ax[4].set_title(f"MAVM {d_max_str}\n{field_str} [{FIELD_UNIT_STR}]",
                        fontsize=plot_opts.font_head_size, fontname=plot_opts.font_name)
        cbar = plt.colorbar(image)
    ...
    for ax_ind,ax_str in enumerate(STRAIN_COMP_STRS):
        ...
        sim_x_min = np.min(coords_common[:,0])
        sim_x_max = np.max(coords_common[:,0])
        sim_y_min = np.min(coords_common[:,1])
        sim_y_max = np.max(coords_common[:,1])
        
        strain_diff_avg = (sim_strain_grid_avg[:,:,ax_ind] 
                           - exp_strain_grid_avg[:,:,ax_ind])
        ...
        if scale_cbar:
            image = ax.imshow(exp_strain_grid_avg[:,:,ax_ind],
        ...
        if scale_cbar:
            image = ax.imshow(sim_strain_grid_avg[:,:,ax_ind],
                                extent=(sim_x_min,sim_x_max,sim_y_min,sim_y_max),
                                vmin = strain_color_min,
                                vmax = strain_color_max)
        else:
