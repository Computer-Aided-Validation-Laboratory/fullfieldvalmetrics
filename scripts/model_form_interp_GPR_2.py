...
for d_type in d_types:
    ...
    with torch.no_grad(), gpytorch.settings.fast_pred_var():
        ...
        pred_mean_train = pred_mean_norm * y_std + y_mean
        lower_train = lower_norm.numpy() * y_std + y_mean
        upper_train = upper_norm.numpy() * y_std + y_mean

    print("\nTraining point predictions:")
    pred_df = pd.DataFrame({
        "Measured": d_vals,
        "Predicted": pred_mean_train,
        "Lower95": lower_train,
        "Upper95": upper_train
    })
    ...
    # Normalise grid using training mean/std
    ...
    with torch.no_grad(), gpytorch.settings.fast_pred_var():
        ...
        pred_mean_norm = observed_pred.mean.numpy()
        pred_lower_norm, pred_upper_norm = observed_pred.confidence_region()
        
        # Denormalise grid using training mean/std
        pred_mean = pred_mean_norm * y_std + y_mean
        pred_lower = pred_lower_norm.numpy() * y_std + y_mean
        pred_upper = pred_upper_norm.numpy() * y_std + y_mean
    ...
    ax.plot_surface(Xg, Yg, pred_lower, color='grey', alpha=interval_alpha, edgecolor='none', label='95% PI')
    ax.plot_surface(Xg, Yg, pred_upper, color='grey', alpha=interval_alpha, edgecolor='none', label='95% PI')

    ax.set_xlabel(r'$x$ (m)', labelpad=10)
    ax.set_ylabel(r'$y$ (m)', labelpad=10)
    ax.set_zlabel(r'Predicted $d$ [$^\circ C$] with 95% PI', labelpad=15)
    ...
    ax.set_title(r'Measured $d$ vs predicted $d$ for ' +  f'"{d_type}" with 95% CI')
    fig.legend()
    ax.grid(True)
