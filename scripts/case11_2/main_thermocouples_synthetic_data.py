...
def main() -> None:
    ...
    EXP_DIR = Path.cwd() / "synthetic_data/case11_2/pyvale-output"
    SIM_DIR = Path.cwd() / "synthetic_data/case11_2/pyvale-output"
    num_tc = 36

    epistemic_upper = 5.0

    sens_ax_labels = (r"Temp. [$^{\circ}C$]",)*num_tc + (r"Coil RMS Voltage [$V$]",)
    sens_tags = ("Temp",)*num_tc + ("Volts",)
    sens_num = len(sens_tags)

    # save_path = Path.cwd() / "images_pointsensors_pulse25X_v4"
    save_path = Path.cwd() / "synthetic_data/case11_2/pyvale-output/valid"
    ...
    sim_keys = {f"TC{n}": n + 1 for n in range(1,  num_tc + 1)}
    ...
    exp_keys = {f"TC{n}": n - 1 for n in range(1,  num_tc + 1)}
    ...
    exp_epis_err = np.ones(num_tc) * epistemic_upper
    print(exp_epis_err.shape)
